...
| Provisioner down | Creates for *new* groups fail their pre-flight (fail-closed, clear error, retryable); existing groups deploy normally because ensure short-circuits on a converged stamp cached... **no** - keep it simple: ensure always round-trips; the provisioner is two replicas behind a Service, same availability posture as the API. |
