...
| `name` | yes | Logical workload name (DNS-1123). `{name}-{group}` must fit in 63 characters together - see `naming` on `GET /v1/functions/info`. |
...
sequenceDiagram
    ...
    U->>API: POST /v1/groups/{group}/functions (git, runtime, ...)
...
```json
{
  ...
  "status": "Pending",
  "regions": [],
  "statusUrl": "/v1/groups/team/functions/image-resizer"
}
```

Then `GET /v1/groups/team/functions/image-resizer` once Ready:
...
>   const { ticket } = await (await fetch("/v1/stream-tickets", {
>     method: "POST",
...
> };
>
> const base = `/v1/groups/${group}/functions/${name}`;
...
`PUT /v1/groups/{group}/{containers|functions}/{name}`.
...
```
POST /v1/groups/{group}/functions/{name}/build   ->   202 Accepted
```
...
And `GET /v1/groups/team/functions` to list the group's functions - general
...
`GET /v1/functions/info` publishes it in both vocabularies (`statuses.workload` and
