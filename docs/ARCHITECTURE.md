...
| `GET` | `/api/v1/groups/{group}/{type}/{name}/status` | **The endpoint to poll.** Only what is live: the same `overallStatus` as the full GET, and per site the KSVC status, `revision`, `replicas`, summed `usage`, and a per-pod breakdown of that usage (`pods[]`, each with `pod`, `revision` and its own `usage`). Fans out to **all sites**, like the full GET. Carries **none** of the desired-state config - a client already has it and it cannot change on its own - which is what keeps a two-second refresh from re-reading the backing Secret on every tick. For a function the build is still read, because that is what reports `Building`; `build` is null for a container. Scaled-to-zero → `replicas: 0`, `usage: null`, empty `pods`. Same `404`/`503` rules as the full GET. |
