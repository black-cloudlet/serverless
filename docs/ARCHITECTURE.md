...
| `GET` | `/api/v1/groups/{group}/{type}/{name}/status` | **The endpoint to poll, and the only one carrying live usage.** The same `overallStatus` as the full GET; workload-wide `replicas` and `usage` totals; and per site the KSVC status, `revision`, `replicas`, `usage`, and a per-pod breakdown of it (`pods[]`, each with `pod`, `revision`, `usage`). Fans out to **all sites**. Carries **none** of the desired-state config - a client already has it and it cannot change on its own - which is what keeps a two-second refresh from re-reading the backing Secret on every tick. For a function the build is still read, because that is what reports `Building`; `build` is null for a container. Totals are summed from the **raw** per-site figures, so they need not equal the sum of the rounded ones shown (two pods at 0.5m each render `"0m"` under a site reading `"1m"`) - render `usage`, never a client-side sum of `pods[]`. A total is **null** if any site could not be measured, rather than a partial figure that still reads as authoritative. Scaled-to-zero → `replicas: 0`, `usage: null`, empty `pods`. Same `404`/`503` rules as the full GET. |
