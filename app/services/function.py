...
class FunctionService:
    ...
    async def accept(self, spec: FunctionCreate, user: Principal, background) -> WorkloadResponse:
        ...
        return self._engine.accepted(OFFERING_FUNCTION, spec.name, host, runtime=spec.runtime)

    async def accept_update(self, name: str, spec: FunctionUpdate, user: Principal, background) -> WorkloadResponse:
        await self._engine.load_existing(name, OFFERING_FUNCTION, user)
        background.add_task(self._engine.run, self.update, name, spec, user)
        return self._engine.accepted(
            OFFERING_FUNCTION, name, self._engine.host_for(name, spec.hostname, user)
        )
    ...
    async def get(self, name: str, user: Principal) -> WorkloadResponse:
        return await self._engine.get(OFFERING_FUNCTION, name, user)

    async def delete(self, name: str, user: Principal) -> None:
        await self._engine.delete(OFFERING_FUNCTION, name, user)
