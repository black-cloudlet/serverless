...
Layered by what each module may import: domain (pydantic only), then cluster
(+ kubernetes). There is no web layer - that is ``cloudlet_apis.web``, behind an
extra the controller does not install. ``tests/test_layering.py`` enforces it.
"""
