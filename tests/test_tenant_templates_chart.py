...
the provisioner's ClusterRole must cover. Each is correct today by agreement,
...
from common.labels import LABEL_MANAGED_BY, PROVISIONER_VALUE
from common.names import NAMESPACE_SUFFIX
from provisioner.templates import PLACEHOLDERS, TEMPLATE_KINDS
...
def test_the_ca_policy_matches_the_label_the_provisioner_actually_stamps():
    ...
    assert f"{LABEL_MANAGED_BY}: {PROVISIONER_VALUE}" in policy
...
def test_the_two_services_hold_different_identities():
    ...
    values = _values()
    assert (
        values["tenantNamespaces"]["provisioner"]["certificate"]["user"]
        != values["certificate"]["user"]
    )


def test_the_chart_emits_exactly_the_placeholders_the_provisioner_substitutes():
    ...
    assert emitted == set(PLACEHOLDERS), (
        f"the chart emits {sorted(emitted)}; the provisioner substitutes {sorted(PLACEHOLDERS)}"
    )
